...
const AdminTemplatesPage = () => {
  ...
  useEffect(() => {
    ...
    if (templates.some((template) => template._id === selectedTemplateId)) return;
  ...
  const selectedTemplate = selectedTemplateId === "new" ? null : templates.find((template) => template._id === selectedTemplateId);
  const selectedDraft = selectedTemplateId === "new" ? newDraft : drafts[selectedTemplateId];
      ...
      <div className="container py-4 admin-templates">
        ...
        <div className="templates-split">
          <div className="template-list">
            <button
              className={`template-list-item ${selectedTemplateId === "new" ? "active" : ""}`}
              type="button"
              onClick={() => setSelectedTemplateId("new")}
            >
              <div className="template-list-title">New Template</div>
              <div className="template-list-meta text-muted">Create a fresh course flow.</div>
            </button>
            {hasTemplates &&
              templates.map((template) => {
                const draft = drafts[template._id];
                if (!draft) return null;
                const itemCount = draft.courses.reduce((sum, course) => sum + course.selectedItemNames.length, 0);
                return (
                  <button
                    key={template._id}
                    className={`template-list-item ${selectedTemplateId === template._id ? "active" : ""}`}
                    type="button"
                    onClick={() => setSelectedTemplateId(template._id)}
                  >
                    <div className="template-list-title">{draft.name || "Template"}</div>
                    <div className="template-list-meta text-muted">
                      {draft.courses.length} courses · {itemCount} items
                    </div>
                  </button>
                );
              })}
          ...
          <div className="template-editor">
            ...
            {selectedTemplateId !== "new" && selectedTemplate && selectedDraft && (
              <div className="template-card">
                <div className="template-card-header">
                  <h3 className="h5 mb-1">{selectedDraft.name || "Template"}</h3>
                  <p className="text-muted mb-0">Edit course structure and suggested items.</p>
                </div>
                <div className="template-card-body">
                  <div className="row g-3">
                    <div className="col-12">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setDrafts((prev) => ({
                            ...prev,
                            [selectedTemplateId]: { ...selectedDraft, name: e.target.value },
                          }))
                        }
                      />
                    </div>
                    <div className="col-12">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setDrafts((prev) => ({
                            ...prev,
                            [selectedTemplateId]: { ...selectedDraft, description: e.target.value },
                          }))
                        }
                      />
                    </div>
                    <div className="col-12">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setDrafts((prev) => ({
                            ...prev,
                            [selectedTemplateId]: { ...selectedDraft, sortOrder: Number(e.target.value) },
                          }))
                        }
                      />
                    </div>
                  ...
                  <div className="course-list">
                    <div className="text-uppercase text-muted small mb-2">Courses</div>
                    {selectedDraft.courses.map((course, index) => (
                      <div key={`${course.name}-${index}`} className="course-card">
                        <div className="course-row">
                          <input
                            className="form-control"
                            value={course.name}
                            onChange={(e) => updateCourse(selectedTemplateId, index, { name: e.target.value })}
                            placeholder="Course name"
                          />
                          <div className="course-meta text-muted small">
                            {course.selectedItemNames.length} items selected
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() => removeCourse(selectedTemplateId, index)}
                          >
                            Remove
                          </button>
                        </div>
                            ...
                            <div className="col-12 col-md-4">
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  if (nextMode === "choice" && !course.maxChoices) updates.maxChoices = 1;
                                  if (nextMode === "share" && !course.shareCount) updates.shareCount = 2;
                                  updateCourse(selectedTemplateId, index, updates);
                                }}
                              >
                                <option value="fixed">Fixed</option>
                                <option value="choice">Choice</option>
                            ...
                            {course.selectionMode === "choice" && (
                              <div className="col-12 col-md-4">
                                ...
                                <input
                                  ...
                                  value={course.maxChoices ?? 1}
                                  onChange={(e) =>
                                    updateCourse(selectedTemplateId, index, {
                              ...
                              {categories.map((category) => {
                                ...
                                return (
                                  <button
                                    ...
                                    type="button"
                                    className={`category-chip ${selected ? "active" : ""}`}
                                    onClick={() => toggleDefaultCategory(selectedTemplateId, index, category.name)}
                                  >
                                    {category.name}
                                  </button>
                                );
                                        ...
                                        return (
                                          <div key={item._id} className="col-12 col-md-6">
                                            <button
                                              className={`menu-item-card ${selected ? "selected" : ""}`}
                                              type="button"
                                              onClick={() => toggleCourseItem(selectedTemplateId, index, item.name)}
                                            >
                                              <div className="d-flex justify-content-between align-items-start gap-2">
                                                <div>
                                                  <h4 className="h6 mb-1">{item.name}</h4>
                    ...
                    <button className="btn btn-outline-secondary btn-sm" type="button" onClick={() => addCourse(selectedTemplateId)}>
                      Add Course
                    </button>
                  </div>
                </div>
                <div className="template-card-footer">
                  <button className="btn btn-outline-secondary" onClick={() => saveTemplate(selectedTemplateId)}>
                    Save Changes
                  </button>
                  <button className="btn btn-outline-danger" onClick={() => deleteTemplate(selectedTemplateId)}>
                    Delete
                  </button>
                </div>
              </div>
