...
const emptyTemplate = (): TemplateDraft => ({
  ...
  description: "",
  sortOrder: 0,
  courses: [{ name: "Course 1", selectedItemNames: [] }],
});
...
const AdminTemplatesPage = () => {
  ...
  const addCourse = (templateId: string | "new") => {
    if (templateId === "new") {
      setNewDraft((prev) => ({ ...prev, courses: [...prev.courses, { name: "Course", selectedItemNames: [] }] }));
      return;
    }
    setDrafts((prev) => ({
      ...prev,
      [templateId]: {
        ...prev[templateId],
        courses: [...prev[templateId].courses, { name: "Course", selectedItemNames: [] }],
      },
    }));
  };
